...
void StBTofGeometry::Init(StMaker *maker, TVolume *starHall)
{
   ...
   mStarHall = starHall;
	...
	//LOG_INFO<<"StBTofGeometry::Init-butter, starhalltitle,name: "<<starHall->GetTitle()<<", "<<starHall->GetName()<<endm;


//   LOG_INFO<<"StBTofGeometry::Init-butter, dateandtime: "<<maker->GetDateTime().GetYear()<<endm;
 ...
 if( maker->GetDateTime().GetYear() >= 2013 ){
	 for(Int_t j=0;j<mModulesInTray;j++){
		...
		if(mBTofSensor[7][j]){
			...
			//LOG_INFO << " StBTofGeometry::Init-butter,imod,index,j: "<<imod<<", "<<mBTofSensor[7][j]->Index()<<", "<<j<<endm;
			imod = -1;
		}
        	if(mBTofSensor[22][j]){
